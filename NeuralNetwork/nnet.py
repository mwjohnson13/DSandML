...
def nnet(learningRate,nHidden,nEpoch,train,test):
    """
    Neural network with single, fully connected hidden layer using stochastic gradient descent to minimize cross-entropy error
    ...
    Parameters
    ----------
    learningRate : number for step size in stochastic gradient descent
    
    nHidden : number of hidden units in single hidden layer
    ...
    while epoch <= nEpoch:
        ...
        for d in range(Xtrain.shape[0]):
            ...
            for h in range(nHidden):
                #print(h)
                o_h.iloc[h,:] = sigmoid(w_i_h[h,:], Xtrain.iloc[d,:])
    ...
    for d in range(Xtest.shape[0]):
        ...
        for h in range(nHidden):
            # print(h)
            o_h.iloc[h, :] = sigmoid(w_i_h[h, :], Xtest.iloc[d, :])
...
np.random.seed(0)
nnet(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5])
